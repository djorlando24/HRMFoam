...
namespace Foam
{
    defineTypeNameAndDebug(pacManTopoFvMesh2, 0);
...
Foam::pacManTopoFvMesh2::calcMotionMask() const
{
...
void Foam::pacManTopoFvMesh2::addZonesAndModifiers()
{
    // Add zones and modifiers for motion action

    if (topoChanger_.size() > 0)
    {
        Info<< "void pacManTopoFvMesh2::addZonesAndModifiers() : "
...
Foam::pacManTopoFvMesh2::pacManTopoFvMesh2(const IOobject& io)
:
...
Foam::pacManTopoFvMesh2::~pacManTopoFvMesh2()
{}


// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

bool Foam::pacManTopoFvMesh2::update()
{
    ...
    bool localMeshChanged = true; // topoChangeMap->morphing();
    ...
    if (globalMeshChanged)
    {
        if (nOldPoints < newPoints.size()) //layer has been removed
        {
            nLayers_++;
        }
        else if (nOldPoints > newPoints.size()) //layer has been added
        {
            nLayers_--;
        }
    }

    Info << "Number of layers: " << nLayers_ << endl;
    ...
    for (label layerI = 1; layerI <= nLayers_; layerI++)  //? what if nLayers = 0 or 1
    {
        ...
        forAll (curCells, i)
        {
            ...
            {
                FatalErrorIn ("bool Foam::pacManTopoFvMesh2::update()")
...
//?? restartable
    ...
    NotImplemented // fix  SBMFPtr_().velocity()
