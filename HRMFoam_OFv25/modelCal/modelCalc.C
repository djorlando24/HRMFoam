...
:
    ...
    runTime_(U.time()),
    Rgas_    ("Rgas",    dimensionSet(0,  2, -2, -1, 0, 0, 0) ,0.0),
    cpGas_   ("cpGas",   dimensionSet(0,  2, -2, -1, 0, 0, 0) ,0.0),
    hDatum_  ("hDatum",  dimensionSet(0,  2, -2,  0, 0, 0, 0) ,0.0),
    psiv_    ("psiv",    dimensionSet(0, -2,  2,  0, 0, 0, 0) ,0.0),
    psil_    ("psil",    dimensionSet(0, -2,  2,  0, 0, 0, 0) ,0.0),
    Pcrit    ("Pcrit",    dimensionSet(1, -1, -2,  0, 0, 0, 0) ,0.0),
    rhoMin_  ("rhoMin",  dimensionSet(1, -3,  0,  0, 0, 0, 0) ,0.0),
     ...
     )
{
   ...
   if (compressible_)
     {
       ...
       dimensionedScalar aL_("aL", dict);
       dimensionedScalar aV_("aV", dict);
     ...
     {
       rhog_ = dimensionedScalar("rhoNCgas", dimDensity, dict);
     }
   ...
   //If features are not desired, set rhoOverwrite = false and/or xOverwrite in thermophysicalProperties
...
tmp<volScalarField> modelCalc::getModel
(
    const volScalarField& pres,
    const volScalarField& h,
...
)
{
  ...
  forAll(mesh_.C(), celli)
  {
   ...
   forAll(mesh_.boundary(), patchI)
   {
      ...
      fvPatchScalarField& patchrhoV = rhoV_.boundaryFieldRef()[patchI];
      fvPatchScalarField& patchrhoL = rhoL_.boundaryFieldRef()[patchI];
      //      fvPatchScalarField& patchpsi = psi_.boundaryField()[patchI];
      fvPatchScalarField& patchmu = mu_.boundaryFieldRef()[patchI];
      fvPatchScalarField& patchxbar = xbar_.boundaryFieldRef()[patchI];
      fvPatchScalarField& patchpSat = pSat_.boundaryFieldRef()[patchI];
      fvPatchScalarField& patchT = T_.boundaryFieldRef()[patchI];
      fvPatchScalarField& patchK = K_.boundaryFieldRef()[patchI];
      fvPatchScalarField& patchalphaFrac = alphaFrac_.boundaryFieldRef()[patchI];
      fvPatchScalarField& patchomega_HRM = omega_HRM_.boundaryFieldRef()[patchI];

      fvPatchScalarField& patchLiq_Mol_a = Liq_Mol_a_.boundaryFieldRef()[patchI];
      fvPatchScalarField& patchVap_Mol_a = Vap_Mol_a_.boundaryFieldRef()[patchI];
	...
	patchLiq_Mol_a[faceI] = thermPoint1->Liq_Mol_a();
