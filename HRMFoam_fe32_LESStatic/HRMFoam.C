...
#   include "setCompressibleDeltaT.H"    // set initial timestep
    ...
    Info<< "\nHRMFoam built with Sigma-Y LES support and dynamic/static mesh switch\n" << endl;
...
#       include "setDeltaT.H"
        ...
        // update mesh--unless the user has opted to include sealing with a static mesh
	bool meshChanged = false;
	
	// the normal situation, where the user has not specified seal with static mesh
	// requires an update
	
	if (!sealWithStaticMesh) meshChanged = mesh.update();
