...
:
    ...
    table_(motionDict_.subDict("needleMotion"))
{
...
bool Foam::injNeedleTopoFvMesh::update()
{
    ...
    // Changing topology by hand
    {
        autoPtr<mapPolyMesh> topoChangeMap2 = topoChanger_.changeMesh(false);
...
//        if (topoChangeMap2->morphing())
    ...
    // Changing topology by hand
    {
        ...
        autoPtr<mapPolyMesh> topoChangeMap3 = topoChanger_.changeMesh(false);

        if (true) // if (topoChangeMap3->morphing())
        {
            if (debug)
            {
