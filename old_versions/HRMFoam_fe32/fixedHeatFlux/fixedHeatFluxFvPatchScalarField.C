...
void Foam::fixedHeatFluxFvPatchScalarField::updateCoeffs()
{
  ...
  const fvPatchField<scalar>& K = this->patch().lookupPatchField
    ...
    const dimensionedScalar   cpGas(thermProps.lookup("cpGas"));
 
    const dimensionedScalar   cpV(thermProps.lookup("cpV"));

    const dimensionedScalar   cpL(thermProps.lookup("cpL"));

    const scalarField cpMix = (1-x)*(1-y)*cpL.value() 
