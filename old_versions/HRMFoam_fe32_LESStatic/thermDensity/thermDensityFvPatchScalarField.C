...
namespace Foam
{
  ...
  void thermDensityFvPatchScalarField::updateCoeffs()
  {
    ...
    dimensionedScalar   rhoMin(thermProps.lookup("rhoMin"));
    ...
    if (comp)
      {
	dimensionedScalar     Rgas(thermProps.lookup("Rgas"));

	dimensionedScalar     cpGas(thermProps.lookup("cpGas"));

	dimensionedScalar    hDatum(thermProps.lookup("hDatum"));

	dimensionedScalar aL
	  (
	   thermProps.lookup("aL")
	   );

	dimensionedScalar aV
	  (
	   thermProps.lookup("aV")
	   );
	...
	forAll(patchField,facei)
	  {                          
	    ...
	    dimensionedScalar rhoNCgas(thermProps.lookup("rhoNCgas"));
